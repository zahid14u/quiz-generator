// 📁 SAVE AS: app/pricing/page.tsx  ← UPDATED with featured reviews + all reviews link
...
export default function PricingPage() {
  ...
  const defaultReviews = [
    { user_name: "Sarah K.", comment: "I used to spend 2 hours making quizzes. Now it takes 10 seconds.", rating: 5, role: "High School Teacher" },
    { user_name: "Ahmed R.", comment: "The True/False and Fill in the Blanks features are perfect for my IT students.", rating: 5, role: "College Lecturer" },
    { user_name: "Maria T.", comment: "My students actually enjoy taking these quizzes!", rating: 5, role: "Primary Teacher" },
  ];

  const reviewsToShow = featuredReviews.length > 0 ? featuredReviews : defaultReviews;
      ...
      <main className="min-h-screen bg-white text-slate-900">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
          ...
          <div className="text-center">
            ...
            <p className="mt-4 text-slate-600">Start free and upgrade anytime for advanced features.</p>
          </div>
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 p-8">
              ...
              <ul className="mt-6 space-y-3 text-sm">
                <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span>MCQ only</li>
                <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span>10 questions max</li>
                <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span>3 quizzes per day</li>
              </ul>
            ...
            <div className="relative rounded-2xl border-2 border-purple-500 bg-purple-50 p-8">
              ...
              <div className="mt-2 flex items-baseline gap-1 flex-wrap">
                <span className="text-4xl font-bold text-purple-700">${proPrice}</span>
                <span className="text-slate-500">{proPeriod}</span>
                {isYearly && (
                  <>
                    <span className="ml-2 text-sm text-slate-400 line-through">$60</span>
                    <span className="ml-2 text-sm text-green-600 font-semibold">Save $24</span>
                    <div className="w-full text-sm text-slate-500">just $3/mo</div>
                  </>
                )}
              </div>
              <ul className="mt-6 space-y-3 text-sm">
                <li className="flex items-center gap-2"><span className="text-purple-600 font-bold">✓</span>All question types</li>
                <li className="flex items-center gap-2"><span className="text-purple-600 font-bold">✓</span>Up to 30 questions per quiz</li>
                <li className="flex items-center gap-2"><span className="text-purple-600 font-bold">✓</span>Unlimited quizzes daily</li>
                <li className="flex items-center gap-2"><span className="text-purple-600 font-bold">✓</span>Priority support</li>
              </ul>
              ...
              >
                {isLoadingCheckout ? "Preparing checkout..." : `Upgrade to Pro — $${proPrice}${proPeriod}`}
              </button>
              <p className="mt-2 text-center text-xs text-slate-500">Cancel anytime · Secure payment via Lemon Squeezy</p>
              <p className="mt-1 text-center text-xs text-slate-500">
                Already Pro?{" "}
                <Link href="/login" className="text-purple-600 hover:underline">Login here →</Link>
              </p>
            </div>
          </div>
          ...
          <div className="mt-12 rounded-lg bg-green-50 border border-green-200 p-6 text-center">
            ...
            <p className="mt-2 text-sm text-green-600">Not satisfied? Full refund within 7 days, no questions asked.</p>
          </div>
          ...
          <div className="mt-20">
            ...
            <div className="mt-8 text-center">
              <Link href="/generate" className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-8 py-3 text-sm font-semibold text-white transition hover:bg-green-700">
                🎁 Try Pro Demo on Generator →
              </Link>
              <p className="mt-2 text-xs text-slate-500">No payment needed · Experience all Pro features live</p>
            </div>
          </div>
          ...
          <div className="mt-20">
            ...
            {featuredReviews.length === 0 && (
              <p className="text-center text-sm text-slate-400 mt-2">Showing sample reviews</p>
            )}
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              {reviewsToShow.map((review, index) => (
                <div key={index} className="rounded-xl bg-slate-50 p-6">
                  <div className="text-yellow-500 text-sm">{"⭐".repeat(review.rating)}</div>
                  <p className="mt-3 text-sm text-slate-700 italic">"{review.comment}"</p>
                  <div className="mt-4">
            ...
            <div className="mt-8 text-center flex items-center justify-center gap-4 flex-wrap">
              <Link href="/reviews" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                📖 Read All Reviews
              </Link>
              <Link href="/review" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                ⭐ Leave a Review
              </Link>
            </div>
          </div>
          ...
          <div className="mt-20">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              {[
                { q: "Can I cancel anytime?", a: "Yes, cancel with one click from your Lemon Squeezy account. No questions asked." },
                ...
                { q: "What is the refund policy?", a: "We offer a 7-day money back guarantee. Contact support for a full refund." },
                { q: "How do I cancel my subscription?", a: "Log into your Lemon Squeezy account and click Cancel. Access continues until end of billing period." },
              ].map((f) => (
          ...
          <div className="mt-20 rounded-2xl bg-slate-900 py-16 text-center text-white">
            ...
            <p className="mt-4 text-slate-300">Join teachers using QuizAI to create engaging quizzes in seconds.</p>
            <button
              onClick={handleUpgrade}
              disabled={isLoadingCheckout}
